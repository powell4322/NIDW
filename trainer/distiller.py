    ...
    def test(self, test_watermark=False):
        try:
            wb_model = torch.load(os.path.join(
                self.export_root, 'models', 'best_acc_model.pth'), weights_only=False).get(STATE_DICT_KEY)
            self.model.load_state_dict(wb_model)
        except:
            wb_model = torch.load(os.path.join(self.export_root, 'models', 'last_model.pth'), weights_only=False)
            self.model.load_state_dict(wb_model)
